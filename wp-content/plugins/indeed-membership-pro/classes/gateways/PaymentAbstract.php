...
abstract class PaymentAbstract
{
	...
	*/
    ...
    public function __construct()
    {
        if ( empty( $this->paymentType ) ){
            ...
            $this->errors[] = __( ' Payment process - Error - Payment Type is not set by Developer.', 'ihc' );
            \Ihc_User_Logs::write_log( __( ' Payment process - Error - Payment Type is not set by Developer. Process stop!', 'ihc'), 'payments' );
            return false;
        }
        ...
        if ( empty( $this->paymentSettings ) ){
            $this->stopProcess = true;
            $this->errors[] = __( 'Payment process - Payment Settings are not set by Administrator.', 'ihc' );
            \Ihc_User_Logs::write_log( $this->paymentTypeLabel . __( ': Payment process - Error - Payment Settings are not set by Administrator.', 'ihc'), 'payments' );
            return false;
        }
	...
	* Set Data provided from Payment step
    ...
    public function setInputData( $params=[] )
    {
        ...
        \Ihc_User_Logs::write_log( $this->paymentTypeLabel . __( ': Set Input data - Values: ', 'ihc') . serialize( $params ), 'payments' );
    ...
    protected function setLevelData()
    {
        ...
        if ( empty( $levelsData[ $this->inputData['lid'] ] ) ){
            $this->levelData = [];
        } else {
            $this->levelData = $levelsData[ $this->inputData ['lid'] ];
        }
        \Ihc_User_Logs::write_log( $this->paymentTypeLabel . __( ': Set Level Data - Values: ', 'ihc') . serialize( $this->levelData ), 'payments' );
        return $this;
    }
    ...
    */
    protected function setDefaultRedirect()
    {
        ...
        $url = apply_filters('ihc_register_redirect_filter', $url, $this->inputData['uid'], $this->inputData['lid']);
    ...
    public function check()
    {
        	...
        	\Ihc_User_Logs::write_log( $this->paymentTypeLabel . __( ': Check process - Start.', 'ihc'), 'payments');

        if ( empty( $this->inputData ) ){
            \Ihc_User_Logs::write_log( $this->paymentTypeLabel . __( ': Check process - Error, Input Data is not provided.', 'ihc'), 'payments' );
            $this->stopProcess = true;
            $this->errors[] =  __( "Input Data is not provided.", 'ihc' );
            return $this;
        }
        \Ihc_User_Logs::set_user_id(@$this->inputData['uid']);
        \Ihc_User_Logs::set_level_id(@$this->inputData['lid']);

        if ( empty( $this->inputData['uid'] ) ){
            \Ihc_User_Logs::write_log( $this->paymentTypeLabel . __( ': Check process - Error, User ID is not provided.', 'ihc'), 'payments');
            $this->stopProcess = true;
            $this->errors[] =  __( "User ID is not provided.", 'ihc' );
            return $this;
        }
        if ( empty( $this->inputData['lid'] ) ){
            \Ihc_User_Logs::write_log( $this->paymentTypeLabel . __( ': Check process - Error, Level ID is not provided.', 'ihc'), 'payments');
            $this->stopProcess = true;
            $this->errors[] =  __( "Level ID is not provided.", 'ihc' );
            return $this;
        }
        if ( empty( $this->levelData ) ){
            \Ihc_User_Logs::write_log( $this->paymentTypeLabel . __(': Check process - Error, Level details are not available.', 'ihc'), 'payments');
            $this->stopProcess = true;
            $this->errors[] =  __( "Level details are not available.", 'ihc' );
            return $this;
        }
        if ( empty( $this->inputData['currency'] ) ){
            \Ihc_User_Logs::write_log( $this->paymentTypeLabel . __(': Check process - Error, Currency is not set.', 'ihc'), 'payments');
            $this->stopProcess = true;
            $this->errors[] =  __( "Currency is not set.", 'ihc' );
            return $this;
        }


		    do_action( 'ihc_action_after_check', $this->inputData );
        	\Ihc_User_Logs::write_log( $this->paymentTypeLabel . __( ': Check process - Finish.', 'ihc'), 'payments');
	...
	* Prepare payment details and calculate the amount based on each Payment gateway workflow and additional options (coupons, taxes, etc).
    ...
    public function preparePayment()
    {
        ...
        do_action( 'ihc_action_before_prepare_payment', $this->paymentOutputData );
		    \Ihc_User_Logs::write_log( $this->paymentTypeLabel . __(': Prepare Payment process - Start', 'ihc'), 'payments');
        ...
        \Ihc_User_Logs::write_log( $this->paymentTypeLabel . __(': Prepare Payment process - Finish - Set Payment Output data. Values: ', 'ihc') . serialize( $this->paymentOutputData), 'payments');
    ...
    protected function preparePaymentSingle()
    {
        ...
        if ( isset( $this->inputData['ihc_coupon'] ) && $this->inputData['ihc_coupon'] != '' ){
            ...
            if ( $couponObject->isValid() ){
                \Ihc_User_Logs::write_log( $this->paymentTypeLabel . __(': Prepare Payment process - User used the following coupon: ', 'ihc') . $this->inputData['ihc_coupon'], 'payments');
    ...
    protected function preparePaymentRecurring()
    {
        ...
        if ( empty( $this->paymentRules['canDoRecurring'] ) ){
            $this->errors[] = $this->paymentTypeLabel .  __( ": Prepare Payment process - Recurring Payment it's not available for this payment gateway.", 'ihc' );
            $this->stopProcess = true;
            return $this;
        }
        ...
        if ( empty( $intervals ) ){
            \Ihc_User_Logs::write_log( $this->paymentTypeLabel . __(': Prepare Payment process -  Recurring cannot be set with this dates. Error on date intervals! ', 'ihc'), 'payments');
            $this->stopProcess = true;
            return $this;
        }
        ...
        if ( isset( $this->inputData['ihc_coupon'] ) && $this->inputData['ihc_coupon'] != '' ){
		    ...
		    ///!!!! - Coupon Recurring (Forever) este default value si cel mai des folosit, deci as incerca sa respect ordinea operatiilor in code precum deault settings din Dashboard
	...
	* Reeturns Payment details after have been calculated via preparePayment. Those details can be used into Cart section.
    ...
    public function getTrialInterval( $trialAmount=0, $accessTrialType='', $intervalTyppe='', $intervalValue=0 )
    {
        ...
        if ( $this->levelData['access_trial_type'] == 1 && $this->intervalTrialRules['supportCertainPeriod'] ){
            ...
            //$temporary = $this->getIntervalValues( $this->levelData['access_regular_time_value'], $this->levelData['access_regular_time_type'], $this->intervalSubscriptionRules );
            $temporary = $this->getIntervalValues( $this->levelData['access_regular_time_value'], $this->levelData['access_regular_time_type'], $this->intervalTrialRules );
            if ( isset( $temporary['intervalValue'] ) && isset( $temporary['intervalSymbol'] ) ){
                $data = [
     ...
     * @return array
     */
    private function convertInterval( $currentType='', $currentValue=0, $rules=[] )
    {
        switch ( $currentType ){
            case 'D': ////// days to weeks, months or years
              ...
              if ( !empty( $rules['weeksSupport'] ) ){
                  ...
                  if ( isset( $rules['weeksLimit'] ) && $rules['weeksLimit'] != '' && $data['intervalValue'] > $rules['weeksLimit'] ){
                      $data['intervalValue'] = $rules['weeksLimit'];
                  }
              }
              // months
              if ( empty( $data ) && !empty( $rules['monthsSupport'] ) ){
                  ...
                  if ( isset( $rules['monthsLimit'] ) && $rules['monthsLimit'] != '' && $data['intervalValue'] > $rules['monthsLimit'] ){
                      $data['intervalValue'] = $rules['monthsLimit'];
                  }
              }
              // years
              if ( empty( $data ) && !empty( $rules['yearsSupport'] ) ){
                  $data['intervalSymbol'] = 'Y';
                  $data['intervalValue'] = $currentValue / 360;
                  if ( isset( $rules['yearsLimit'] ) && $rules['yearsLimit'] != '' && $data['intervalValue'] > $rules['yearsLimit'] ){
                      $data['intervalValue'] = $rules['yearsLimit'];
                  }
              }
              break;
            ...
            case 'Y': ////// years to days, weeks or months
              ...
              if ( !empty( $rules['daysSupport'] ) ){
                  $data['intervalSymbol'] = 'D';
                  $data['intervalValue'] = $currentValue * 360;
                  if ( $data['intervalValue'] > $rules['daysLimit'] ){
                      $data['intervalValue'] = $rules['daysLimit'];
                  }
              }
              // weeks
              if ( empty( $data ) && !empty( $rules['weeksSupport'] ) ){
                  $data['intervalSymbol'] = 'W';
                  $data['intervalValue'] = $currentValue * 52;
                  if ( $data['intervalValue'] > $rules['weeksLimit'] ){
                      $data['intervalValue'] = $rules['weeksLimit'];
                  }
              }
              // months
              if ( empty( $data ) && !empty( $rules['monthsSupport'] ) ){
                  $data['intervalSymbol'] = 'M';
                  $data['intervalValue'] = $currentValue * 12;
                  if ( $data['intervalValue'] > $rules['monthsLimit'] ){
                      $data['intervalValue'] = $rules['monthsLimit'];
                  }
              }
              break;
    ...
    */
    protected function dynamicPrice( $amount=0 )
    {
        ...
        if ( $dynamicPriceObject->checkPrice( $this->inputData['lid'], $this->inputData['ihc_dynamic_price'] ) ){
            ...
            \Ihc_User_Logs::write_log( $this->paymentTypeLabel . __(' : Dynamic price on - Amount is set by the user @ ', 'ihc') . $amount . $this->inputData['currency'], 'payments');
        }
        return $amount;
    }
    ...
    public function saveOrder()
    {
        ...
        \Ihc_User_Logs::write_log( $this->paymentTypeLabel . __( ': Save Order process - Start.', 'ihc'), 'payments');
        ...
        \Ihc_User_Logs::write_log( $this->paymentTypeLabel . __( ': Save Order process - Order with Values: ' . serialize( $orderData ), 'ihc'), 'payments');
        \Ihc_User_Logs::write_log( $this->paymentTypeLabel . __( ': Save Order process - Completed.', 'ihc'), 'payments');

        //save order metas
        $orderMeta = new \Indeed\Ihc\Db\OrderMeta();
        \Ihc_User_Logs::write_log( $this->paymentTypeLabel . __( ': Save Order process - Save Metas.', 'ihc'), 'payments');
        $exclude = [ 'order_id' ];
        ...
        \Ihc_User_Logs::write_log( $this->paymentTypeLabel . __( ': Save Order process - Finish.', 'ihc'), 'payments');
    ...
    public function chargePayment()
    {
        ...
        \Ihc_User_Logs::write_log( $this->paymentTypeLabel . __( ': Charge Payment process - Start.', 'ihc'), 'payments');
  		  ...
  		  if ( $this->isLevelFree() ){
  			  ...
  			  \Ihc_User_Logs::write_log( $this->paymentTypeLabel . __( ': Charge Payment process - Level is free.', 'ihc'), 'payments');
  			  return $this;
  		  }

  		  // no charge
  		  if ( !empty( $this->paymentOutputData['no_charge'] ) ){
    			  \Ihc_User_Logs::write_log( $this->paymentTypeLabel . __( ': Charge Payment process - No payment for this subscription.', 'ihc'), 'payments');
        ...
        $orderMeta->save( $this->paymentOutputData['order_id'], 'order_identificator', $this->paymentOutputData['order_identificator'] );
        	...
        	\Ihc_User_Logs::write_log( $this->paymentTypeLabel . __( ': Charge Payment process - Finish.', 'ihc'), 'payments');
    ...
    public function webhookPayment()
    {
         do_action( 'ihc_action_before_webhook' );
		     \Ihc_User_Logs::write_log( $this->paymentTypeLabel . __( ': Webhook - Start.', 'ihc'), 'payments');
        ...
        do_action( 'ihc_action_after_webhook' );
		    \Ihc_User_Logs::write_log( $this->paymentTypeLabel . __( ': Webhook  - Finish.', 'ihc'), 'payments');
    ...
    public function processingWebhookData()
    {
        ...
        if ( !isset( $this->webhookData['uid'] ) || !\Ihc_Db::does_user_exists( $this->webhookData['uid'] ) ){
		    ...
		    \Ihc_User_Logs::write_log( $this->paymentTypeLabel . __( ': Processing Webhook Data - Start.', 'ihc'), 'payments');
        ...
        switch ( $this->webhookData['payment_status'] ){
            case 'completed':
                ...
                \Ihc_User_Logs::write_log( $this->paymentTypeLabel . __( ': Processing Webhook Data - Check Order based on transaction_id: ', 'ihc'). $this->webhookData['transaction_id'], 'payments');
                ...
                if ( $orderId && $this->webhookIsTransactionCompleted( $this->webhookData['transaction_id'] ) ){
                   \Ihc_User_Logs::write_log( $this->paymentTypeLabel . __(' Processing Webhook Data - This order has already been completed. Exit.', 'ihc'), 'payments');
                   return;
                }
                ...
                if ( empty( $orderId ) || ( $orderId && $this->webhookIsOrderCompleted( $orderId ) ) ){
                    ...
                    \Ihc_User_Logs::write_log( $this->paymentTypeLabel . __( ': Processing Webhook Data - No Order found. Insert a new one.', 'ihc').json_encode( $orderData), 'payments');
                    $orderId = $this->webhookInsertOrder( $orderData );
                }

      				  if ( empty( $orderId ) ){
      					    \Ihc_User_Logs::write_log( $this->paymentTypeLabel . __( ': Processing Webhook Data - No Order exist or Inserted. Exit.', 'ihc'), 'payments');
      					    return false;
      				  }

                if ( $this->webhookData['payment_details'] && is_array( $this->webhookData['payment_details'] ) ){
                ...
                \Ihc_User_Logs::write_log( $this->paymentTypeLabel . __( ': Processing Webhook Data - Finish - Call completeLevelPayment.', 'ihc'), 'payments');
            ...
            case 'pending':
                ...
                break;
            case 'refund':
                ihc_delete_user_level_relation( $this->webhookData['lid'], $this->webhookData['uid'] );
    ...
    public function webhookInsertOrder( $params=[] )
    {
        ...
        $orderId = $object->do_insert( $orderData, $this->isRecurringLevel() );
		    ...
		    \Ihc_User_Logs::write_log( $this->paymentTypeLabel . __( ': Webhook Insert Order - New Order ID is: ', 'ihc'). $orderId, 'payments');
        ...
        if ( !$parentOrderId ){
			     \Ihc_User_Logs::write_log( $this->paymentTypeLabel . __( ': Webhook Insert Order - No Parent Order have been found. Return ', 'ihc'), 'payments');
           return $orderId;
        }
        $metas = $orderMeta->getAllByOrderId( $parentOrderId ); ///!!!!!!!! NU EIXSTA
        if ( !$metas ){
			      \Ihc_User_Logs::write_log( $this->paymentTypeLabel . __( ': Webhook Insert Order - No Metas for Parent Order have been found. Return', 'ihc'), 'payments');
            return $orderId;
        }
		    \Ihc_User_Logs::write_log( $this->paymentTypeLabel . __( ': Webhook Insert Order - Save additional Order Metas: ', 'ihc').json_encode( $metas), 'payments');
        $exclude = [ 'order_id', 'order_identificator', 'transaction_id' ];
        ...
        $orderMeta->save( $orderId, 'order_referrence', $parentOrderId );
		    ...
		    \Ihc_User_Logs::write_log( $this->paymentTypeLabel . __( ': Webhook Insert Order - Finish ', 'ihc'), 'payments');
        return $orderId;
    }
    ...
    public function refundSubscription( $uid=0, $lid=0 )
    {
        ...
        \Ihc_User_Logs::write_log( $this->paymentTypeLabel . __( ': Refund Subscription - Start.', 'ihc'), 'payments');
    ...
    {
        global $wpdb;
        $query = $wpdb->prepare( "UPDATE {$wpdb->prefix}ihc_user_levels SET status='0' WHERE user_id=%d AND level_id=%d;", $uid, $lid );
        $wpdb->query( $query );
    ...
    public function paymentFailed( $uid=0, $lid=0 )
    {
        ...
        \Ihc_User_Logs::write_log( $this->paymentTypeLabel . __( ': Payment Failed.', 'ihc'), 'payments');
    ...
    public function cancelSubscription( $uid=0, $lid=0, $transactionId='' )
    {
        ...
        \Ihc_User_Logs::write_log( $this->paymentTypeLabel . __( ': Trigger cancel Subscription.', 'ihc'), 'payments');
   ...
   */
    ...
    {
        \Ihc_User_Logs::write_log( $this->paymentTypeLabel . __( ': Complete Payment process - Start.', 'ihc'), 'payments');
        ...
        \Ihc_User_Logs::write_log( $this->paymentTypeLabel . __(": Complete Level Payment - Updated User (".$paymentData['uid'].") Level (".$paymentData['lid'].") expire time.", 'ihc'), 'payments');
        ...
        \Ihc_User_Logs::write_log( $this->paymentTypeLabel . __(': Complete Level Payment - Finish.', 'ihc'), 'payments');
   ...
   /**
    ...
    {
		    \Ihc_User_Logs::write_log( $this->paymentTypeLabel . __(' Payment Webhook: Success Redirect 200. Exit.', 'ihc'), 'payments');
    ...
    {
		   \Ihc_User_Logs::write_log( $this->paymentTypeLabel . __(' Payment Webhook: Fail Redirect 400. Exit.', 'ihc'), 'payments');
