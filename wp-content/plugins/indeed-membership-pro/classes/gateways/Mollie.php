...
class Mollie extends \Indeed\Ihc\Gateways\PaymentAbstract
{
    ...
    public function charge()
    {
        ...
        if ( $this->paymentOutputData['is_recurring'] ){
            ...
            $paymentParams = [
                ...
                "description"     => __('Buy ', 'ihc') . $this->paymentOutputData['level_label'],
            ...
            if ( isset( $this->paymentOutputData['first_amount'] ) && $this->paymentOutputData['first_amount'] !== false ){
                ...
                $paymentParams['description'] = __('Buy ', 'ihc') . $this->paymentOutputData['level_label']
            ...
            $SubscriptionParams = [
                ...
                "description" => __('Sign for ', 'ihc') . $this->paymentOutputData['level_label'] .__(' subscription', 'ihc'),
                "webhookUrl"  => $webhook,
                "method"      => NULL,
                'metadata'    => [
                                'order_id'              => $this->paymentOutputData['order_id'],
            ...
            if ( isset( $subscriptionObject->id  ) ){
                $orderMeta = new \Indeed\Ihc\Db\OrderMeta();
                $orderMeta->save( $this->paymentOutputData['order_id'], 'subscription_id', $subscriptionObject->id );
            }
            if ( isset( $customer->id ) ){
                $orderMeta = new \Indeed\Ihc\Db\OrderMeta();
                $orderMeta->save( $this->paymentOutputData['order_id'], 'customer_id', $customer->id );
            }

            // save customer id as order_identificator
            //$orderMeta->save( $this->paymentOutputData['order_id'], 'order_identificator', $subscriptionObject->customerId  );
        } else {
            // single payment
    ...
    public function webhook()
    {
		...
		\Ihc_User_Logs::write_log( $this->paymentTypeLabel . __("  Payment Webhook - transaction_id is ", 'ihc') . $_POST["id"], 'payments');

		$payment = $mollie->payments->get( $transactionId );
		\Ihc_User_Logs::write_log( $this->paymentTypeLabel . __("  Payment Webhook - Payment details ", 'ihc') .  serialize($payment), 'payments');
        ...
        $this->webhookData = [
                                ...
                                'amount'                      => $amount,
                                'currency'                    => $currency,
                                'payment_details'             => $payment,//$responseData,
                                'payment_status'              => '',
        ];
        ...
        if ($payment->isPaid() && !$payment->hasRefunds() && !$payment->hasChargebacks()) {
			...
			\Ihc_User_Logs::write_log( $this->paymentTypeLabel . __("  Payment Webhook - Order Identificator is ", 'ihc') .  $this->webhookData['order_identificator'], 'payments');
