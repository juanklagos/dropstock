...
class PayPalStandard extends \Indeed\Ihc\Gateways\PaymentAbstract
{
    ...
    public function charge()
    {
        if ( $this->paymentSettings['ihc_paypal_sandbox'] ){
            ...
            \Ihc_User_Logs::write_log( $this->paymentTypeLabel . __(': set Sandbox mode', 'ihc'), 'payments');
        } else {
            // live
            $this->redirectUrl = 'https://www.paypal.com/cgi-bin/webscr';
            \Ihc_User_Logs::write_log( $this->paymentTypeLabel . __(': set Live mode', 'ihc'), 'payments');
        }
    ...
    public function webhook()
    {
        ...
        \Ihc_User_Logs::write_log( $this->paymentTypeLabel . __( ': Payment Webhook - Extract data from response.', 'ihc'), 'payments');
        ...
        $getMagicQuotesExists = function_exists( 'get_magic_quotes_gpc' ) ? true : false;
        foreach ($PostData as $key => $value) {
          if ( $getMagicQuotesExists && get_magic_quotes_gpc() == 1 ) {
        ...
        if ( $this->paymentSettings['ihc_paypal_sandbox'] ){
          ...
          \Ihc_User_Logs::write_log( $this->paymentTypeLabel . __(': Payment Webhook -  Set Sandbox mode.', 'ihc'), 'payments');
        } else {
          $paypalUrl = "https://www.paypal.com/cgi-bin/webscr";
          \Ihc_User_Logs::write_log( $this->paymentTypeLabel . __(': Payment Webhook -  Set live mode.', 'ihc'), 'payments');
        }

        $ch = curl_init( $paypalUrl );
        if ($ch == FALSE) {
            \Ihc_User_Logs::write_log( $this->paymentTypeLabel . __(': Payment Webhook -  End Process. No CURL Enabled on this server. ', 'ihc'), 'payments');
            exit;
        }
        \Ihc_User_Logs::write_log( $this->paymentTypeLabel . __( ': Payment Webhook -  Send cURL request to PayPal.', 'ihc'), 'payments');
        ...
        if (curl_errno($ch) != 0){ // cURL error
          \Ihc_User_Logs::write_log( $this->paymentTypeLabel . __(": Payment Webhook -  cURL error - can't connect to PayPal to validate IPN message: ", 'ihc') . curl_error($ch) . PHP_EOL, 'payments');
          curl_close($ch);
          exit; /// out
        } else {
        ...
        if ( strcmp( $res, "VERIFIED") == 0 ) {
          \Ihc_User_Logs::write_log( $this->paymentTypeLabel . __( ": Payment Webhook - cURL request Verified.", 'ihc'), 'payments');
          if ( !$this->webhookData['uid'] || $this->webhookData['lid']<0 ){
              \Ihc_User_Logs::write_log( $this->paymentTypeLabel . __(': Payment Webhook -  No user id or level id for this transaction. Exit.', 'ihc'), 'payments');
              exit;
          }

          \Ihc_User_Logs::write_log( $this->paymentTypeLabel . __(': Payment Webhook - Response: '.json_encode($_POST), 'ihc'), 'payments');
          \Ihc_User_Logs::set_user_id( $this->webhookData['uid'] );
          \Ihc_User_Logs::set_level_id( $this->webhookData['lid'] );
          \Ihc_User_Logs::write_log( $this->paymentTypeLabel . __(": Payment Webhook -  set user id @ ", 'ihc') . $this->webhookData['uid'], 'payments');

          if (isset($_POST['payment_status'])){
              \Ihc_User_Logs::write_log( $this->paymentTypeLabel . __( ": Payment Webhook - payment_status is ", 'ihc') . $_POST['payment_status'], 'payments');
              switch ($_POST['payment_status']){
                ...
                case 'Canceled':
                  ...
                  break;
			        ...
			        \Ihc_User_Logs::write_log( $this->paymentTypeLabel . __( ": Payment Webhook - txn_type is ", 'ihc') . $_POST['txn_type'], 'payments');
              if (!empty($_POST['period1'])){
				          if (isset($_POST['mc_amount1']) && (float)$_POST['mc_amount1']==0){
					            \Ihc_User_Logs::write_log( $this->paymentTypeLabel . __( ": Payment Webhook - period1 is ", 'ihc') . $_POST['period1'], 'payments');
                  		$this->webhookData['payment_status'] = 'completed';
				          } else {
                      ...
                      //$this->webhookData['payment_status'] = 'pending';
                      \Ihc_User_Logs::write_log( $this->paymentTypeLabel . __( ": Payment Webhook - wait for a new response with  payment_status = Completed", 'ihc'), 'payments');
              	  }
              } else if (isset($_POST['mc_amount1']) && (int)$_POST['mc_amount1']==0){
				          \Ihc_User_Logs::write_log( $this->paymentTypeLabel . __( ": Payment Webhook - mc_amount1 is ", 'ihc') . $_POST['mc_amount1'], 'payments');
    ...
    {
        do_action( 'ump_paypal_user_do_refund', $uid, $lid, @$_POST['txn_id'] );
    }
